...
进入Ubuntu系统后，同时按下`Ctrl`+<font color=#A52A2A>`Alt`</font>+`T`打开终端`Terminal`窗口。

在终端窗口中输入以下<font color=#A52A2A>**`1`**</font>条命令打开Jupyter Notebook编译环境：

	jupyter notebook

键入`Enter`回车键后等待，浏览器会自动打开如下地址：

![](https://i.imgur.com/PZYQqSc.png)

点击页面右上方区域按钮`new`->`python3`

将`exper1.txt`文件中代码复制入`In[]:`后光标中，键入`Shift`+`Enter`运行。

![](https://i.imgur.com/PFyjMn9.png)

代码文件`exper1.txt`中实现了以下步骤，详见**代码注释**：
...
打开编译环境。如实验一打开Jupyter Notebook，新建`new`->`python3`交互窗口。

代码文件`exper2.txt`中实现了以下步骤，详见**代码注释**：
...
代码文件`exper3.txt`中实现了以下步骤，详见**代码注释**：
...
- 首先粘贴`part1`部分代码，运行`part1`部分代码（可能要运行几分钟之后才有结果输出，耐心等待一下）：
	...
	1. 学习和预测。在给定的训练样本上运行以上神经网络，观察交叉熵（Cross Entropy）`error`值的变化，并在待测样本上做出预测。 
...
随着迭代的进行，神经网络在数据集上的交叉熵误差越来越小，代表正在慢慢拟合训练数据。

最后在测试集上的测试准确率为`91.02%`
